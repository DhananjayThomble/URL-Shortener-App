...
/* ============================================================
   The :id route on the bio-pages controller parses its id as a UUID at the
   edge (@Param("id", ParseUUIDPipe)), same as LinksController.

   The bug this pins (issue #533): DELETE /bio-pages/not-a-uuid sent the raw
   string straight into a Drizzle `where id = $1` query, and Postgres raised
   `invalid input syntax for type uuid` (SQLSTATE 22P02). That code is not in
   PostgresErrorFilter's map, so it surfaced as a 500 for what is really a
   malformed request. ParseUUIDPipe rejects a malformed id with a clean 400
...
function paramPipesFor(methodName: keyof BioPagesController): unknown[] {
  const meta =
    Reflect.getMetadata(ROUTE_ARGS_METADATA, BioPagesController, methodName as string) ?? {};
  const entries = Object.values(meta) as Array<{ pipes?: unknown[] }>;
  return entries.flatMap((entry) => entry.pipes ?? []);
}

function hasParseUuidPipe(methodName: keyof BioPagesController): boolean {
  return paramPipesFor(methodName).some(
    (pipe) => pipe === ParseUUIDPipe || pipe instanceof ParseUUIDPipe,
  );
}

describe("BioPagesController :id routes reject a non-UUID before the DB", () => {
  it("remove parses its :id with ParseUUIDPipe", () => {
    expect(hasParseUuidPipe("remove")).toBe(true);
  });
});
