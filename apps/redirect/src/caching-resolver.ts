...
/* ============================================================
   ...
   Delete and abuse-flag ARE invalidated synchronously (#470, #426),
   unlike edit: the projection outbox (apps/worker/src/jobs/outbox.ts)
   deletes this resolver's cache key from the SAME CacheStore when a
   link's outbox row is a "delete" op, or an "upsert" op whose payload
   carries a flag marker, via the shared linkCacheKey() format from
   @snapurl/cache. This resolver does not call del() itself — it never
   sees a delete or a flag, only resolve() calls — the invalidation
   happens entirely on the write side, from the API/worker, keeping
   this file and the redirect hot path unchanged. That bust is only
   observable when the CacheStore is actually shared across processes
   (CACHE_DRIVER=redis or dynamodb); under the single-node default
   (CACHE_DRIVER=memory, one Map per process) the worker's del() lands
   on its own private store, not the redirect's, so a deleted or
   flagged link there still relies on the short TTL to stop serving
   the old response, same as before.
...
export class CachingLinkResolver implements LinkResolver {
  ...
  async resolve(host: string, slug: string): Promise<ResolvedLink | null> {
    ...
    // (#470) or abuse flagging (#426) — one key format, shared via
    // @snapurl/cache, so the two never disagree about which key names a
    // given link's entry.
