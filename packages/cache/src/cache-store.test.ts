...
/* linkCacheKey is the one place the hot-link cache key format is defined.
   CachingLinkResolver (apps/redirect) and drainOutbox's delete-row bust
   (apps/worker, #470) both call this function rather than building the
   string themselves, so a normalisation bug here would silently desync
   what the two sides think a link's cache entry is called. */
